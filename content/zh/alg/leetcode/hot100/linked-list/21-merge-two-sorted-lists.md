...
目标不是把值“重新排一遍”，而是沿着两条已经有序的链，持续拿出当前最小节点。  
所以这里最重要的信息是：**每条链的头部就是该链当前最小值。**

#### Step 2：当前部分答案最少要记住什么？

如果我们想边走边构造结果链，就至少要维护三件事：
...
如果没有哨兵节点，你第一次接入结果链时就得反复判断：

- 这是第一个节点吗？
- 结果链头是谁？

加一个虚拟头之后，所有“追加节点”的动作都统一成：
...
这样整个过程就不需要专门为“头节点”写分支。

#### Step 4：一轮循环到底做什么？

当 `p1` 和 `p2` 都非空时，当前最小值只可能来自两个头节点之一。  
所以每一轮都只做一个判断：
...
#### Step 5：接上一个节点后，哪些状态要一起更新？

假设当前接上的是 `p1`，那就要同步做三件事：
...
> `tail` 负责结果链尾部，`p1/p2` 负责两条输入链当前头部；每次只接较小头节点，最后把剩余段整段挂上。

这就是后面 `23. Merge k Sorted Lists` 的地基。
