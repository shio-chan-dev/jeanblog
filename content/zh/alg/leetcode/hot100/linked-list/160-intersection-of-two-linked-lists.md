...
如果两条链表在进入公共尾巴前，私有前缀长度刚好一样，那么双指针同步走就会自然在交点相遇。
...
所以难点不在“怎么判断相等”，而在“怎么把这段长度差抵消掉”。

#### Step 4：把“长度对齐”改写成自动完成的过程
...
等到两者都完成一次跨链遍历后，原来的前缀长度差就被抵消了。
从那一刻起，它们到公共尾巴的剩余距离完全相同。

#### Step 6：循环到底在什么条件下结束？

只有两种情况：

- 两个指针指向同一个真实节点，此时这个节点就是交点
- 两个指针同时变成 `None`，说明根本没有交点
...
一开始两者当然不同步，因为前缀长度不一样。
但当某个指针走到末尾并切到另一条链后，这个长度差会被自动抹平。
于是两个指针会一起进入公共尾巴，并在节点 `8` 相遇。

#### Step 8：把方法压缩成一句模板

160 题可以记成：让两个指针分别走 `A+B` 和 `B+A`，总路程一样后，前缀长度差自然抵消。
