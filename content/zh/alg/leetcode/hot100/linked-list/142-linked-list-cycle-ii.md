...
- 链表确实有环
- 题目要返回的不是“某个环上节点”，而是第一次进入环的那个节点 `b`

所以 142 题比 141 题更严格：
只会判环还不够，还得把“入口”精确找出来。

#### Step 2：先承认一个最直接但不够优的解

最容易写对的办法是哈希集合：

- 从头遍历
- 把访问过的节点引用放进集合
- 第一次遇到“已经见过”的节点，它就是入环点

这个思路非常直观，但空间是 `O(n)`。
题目真正的难点在于：**能不能不额外记住所有节点？**

#### Step 3：第一阶段先只解决“有没有环”

Floyd 的快慢指针刚好可以在 `O(1)` 额外空间下完成判环：
...
- `fast` 或 `fast.next` 先变成 `null`，说明无环
- `slow` 和 `fast` 在环内某处相遇，说明有环

这一步先不急着找入口，只先拿到一个“环内已知点”。

#### Step 4：相遇点到底给了我们什么信息？
...
- 从 `head` 走到入环点，要走 `a` 步
- 从“相遇点”继续走到入环点，按环长度取模以后，也等价于 `a` 步

也就是说，这两个位置到入环点“剩余距离相同”。

#### Step 5：把上面的距离关系翻译成第二阶段

既然头结点和相遇点到入环点的剩余距离相同，那就可以：
...
真正的入环点是值为 `2` 的节点。
快慢指针第一次相遇的位置通常不是 `2`，而是环内别的某个点。

但当我们把一个指针放回 `head`，另一个留在相遇点，再同步前进时，它们会在 `2` 重新相遇。

这就是为什么算法返回的是“入口”，不是“任意环上节点”。

#### Step 7：把方法压缩成一句模板

142 题可以记成：先用 Floyd 拿到一个环内相遇点，再让头指针和相遇点指针同步走，下一次相遇就是入环点。
